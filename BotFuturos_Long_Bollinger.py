...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("trading_futures_long_bb.log"), logging.StreamHandler()]
)
...
class Config:
    ...
    SYMBOL = 'BTCUSDT'
    QTY = 0.002 
    INTERVAL = AsyncClient.KLINE_INTERVAL_15MINUTE # 15m para Bollinger Rebound
    LEVERAGE = 10
    ...
    HISTORY_FILE = "historial_long_bollinger.json"
...
class TradingVisualizer:
    ...
    def generate_dashboard(df: pd.DataFrame, trades: List[Dict], stats: Dict):
        ...
        if 'EMA_200' in df_plot.columns:
            ax1.plot(df_plot.index, df_plot['EMA_200'], color='#f0883e', ls='--', alpha=0.6, label="EMA 200")
        
        # Bandas de Bollinger
        ax1.plot(df_plot.index, df_plot['BBU'], color='#3fb950', ls=':', alpha=0.3)
        ax1.plot(df_plot.index, df_plot['BBL'], color='#f85149', ls=':', alpha=0.3)
        ax1.fill_between(df_plot.index, df_plot['BBL'], df_plot['BBU'], color='#bc8cff', alpha=0.03)
        
        for t in trades:
            try:
                e_t = pd.to_datetime(t['entry_time']).tz_localize('UTC').tz_convert(tz_local)
                ax1.scatter(e_t, t['entry_price'], marker='^', color='#3fb950', s=200, zorder=5)
                if t['status'] == 'CLOSED':
                    x_t = pd.to_datetime(t['exit_time']).tz_localize('UTC').tz_convert(tz_local)
                    color = '#3fb950' if t['pnl_neto'] > 0 else '#f85149'
                    ax1.scatter(x_t, t['exit_price'], marker='v', color=color, s=200, zorder=5)
            except: continue
        ...
        ax5.axis('off')
        summary_text = (
            f"RESUMEN DE SESIÓN: BOLLINGER REBOUND (LONG)\n"
            f"{'='*60}\n"
        ...
        filename = f"reporte_long_bb_{datetime.now().strftime('%H%M')}.png"
...
class EliteTradingEngine:
    ...
    def send_telegram_photo(self, path, stats):
        ...
        caption = (
            f"🚀 <b>EXPERT REPORT: LONG BB SESSION</b>\n"
            f"----------------------------------\n"
    ...
    async def execute_order(self, side, reason, price, time, sl=None, tp=None):
        try:
            ...
            if reason == 'LONG':
                ...
                self.send_telegram_msg(f"🟢 <b>ENTRADA LONG (BB)</b>\nPrecio: ${price:.2f}\nSL: ${sl:.2f} | TP: ${tp:.2f}")
            else:
                t = self.active_trade
    ...
    async def run(self):
        ...
        logger.info("Bot Long Bollinger Rebound Activo (15m)...")
        self.send_telegram_msg("🚀 <b>Bot Long Bollinger Conectado</b>")
        ...
        try:
            ...
            async with bm.futures_multiplex_socket([f"{Config.SYMBOL.lower()}@kline_15m"]) as stream:
                while True:
                    ...
                    if k['x']: # Al cierre de vela
                        klines = await self.client.futures_historical_klines(Config.SYMBOL, Config.INTERVAL, "5 days ago UTC")
                        ...
                        self.data['BBL'] = bb.iloc[:, 0]
                        self.data['BBU'] = bb.iloc[:, 2]
                        ...
                        if not self.active_trade:
                            # Lógica Bollinger Rebound
                            if price > last['EMA_200'] and prev['close'] < prev['BBL'] and last['RSI_14'] > 30:
                                risk = last['ATR'] * Config.ATR_MULTIPLIER
                                await self.execute_order('BUY', 'LONG', price, self.data.index[-1], price - risk, price + (risk * Config.RISK_REWARD_RATIO))
                        else:
                            t = self.active_trade
            ...
            # CIERRE SEGURO Y REPORTE
            logger.info("Generando reporte y cerrando conexiones...")
