...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("trading_futures_long_pro.log"), logging.StreamHandler()]
)
...
class Config:
    ...
    HISTORY_FILE = "historial_long_pro.json"
...
class TradingVisualizer:
    ...
    def generate_dashboard(df: pd.DataFrame, trades: List[Dict], stats: Dict):
        ...
        # Dashboard de 5 niveles (Precio, MACD, StochRSI, RSI, Resumen)
        fig = plt.figure(figsize=(16, 18))
        fig.set_facecolor('#0d1117')
        gs = gridspec.GridSpec(5, 1, height_ratios=[3, 1, 1, 1, 0.8], hspace=0.3)

        # 1. PANEL DE PRECIO + EMA 200
        ...
        for t in trades:
            try:
                ...
                # Triángulo Verde hacia ARRIBA para entrada LONG
                ax1.scatter(e_t, t['entry_price'], marker='^', color='#3fb950', s=200, zorder=5, label='Entrada Long' if 'Entrada Long' not in ax1.get_legend_handles_labels()[1] else "")
                if t['status'] == 'CLOSED':
                    x_t = pd.to_datetime(t['exit_time']).tz_localize('UTC').tz_convert(tz_local)
                    color = '#3fb950' if t['pnl_neto'] > 0 else '#f85149'
                    # Triángulo hacia ABAJO para cierre
                    ax1.scatter(x_t, t['exit_price'], marker='v', color=color, s=200, zorder=5, label='Cierre Long' if 'Cierre Long' not in ax1.get_legend_handles_labels()[1] else "")
            except: continue
        ...
        if 'MACD' in df_plot.columns:
            ax2.plot(df_plot.index, df_plot['MACD'], color='#58a6ff', lw=1, label='MACD')
            ax2.plot(df_plot.index, df_plot['MACDs'], color='#f0883e', lw=1, label='Señal')
        ...
        if 'STOCH_K' in df_plot.columns:
            ...
            ax3.fill_between(df_plot.index, 20, 80, color='#bc8cff', alpha=0.05)
        ...
        ax4.xaxis.set_major_formatter(mdates.DateFormatter('%H:%M', tz=tz_local))
        ...
        ax5.axis('off')
        summary_text = (
            f"RESUMEN DE SESIÓN: LONG ENGINE PRO (1m)\n"
            f"{'='*60}\n"
        ...
        filename = f"reporte_long_live_pro_{datetime.now().strftime('%H%M')}.png"
...
class EliteTradingEngine:
    ...
    def save_history(self):
        ...
        except Exception as e: logger.error(f"Error guardando JSON: {e}")
    ...
    def send_telegram_photo(self, path, stats):
        ...
        caption = (
            f"🚀 <b>EXPERT REPORT: LONG SESSION PRO</b>\n"
            f"----------------------------------\n"
            f"PnL Neto Total: ${stats['net_pnl']:.4f}\n"
    ...
    async def execute_order(self, side, reason, price, time, sl=None, tp=None):
        try:
            ...
            if reason == 'LONG':
                self.active_trade = {'type': 'LONG', 'entry_price': price, 'entry_time': str(time), 'sl': sl, 'tp': tp, 'status': 'OPEN', 'comm_in': comm}
                self.send_telegram_msg(f"🟢 <b>ENTRADA LONG</b>\nPrecio: ${price:.2f}\nSL: ${sl:.2f} | TP: ${tp:.2f}")
            else:
                t = self.active_trade
                # PnL para Long: (Salida - Entrada)
                pnl_bruto = (price - t['entry_price']) * Config.QTY
                ...
                self.send_telegram_msg(f"{emoji} <b>CIERRE LONG</b>\nPnL Neto: ${t['pnl_neto']:.4f}")
    ...
    async def run(self):
        ...
        try:
            ...
            logger.info(f"Apalancamiento configurado a {Config.LEVERAGE}x")
        except: pass

        logger.info("Bot Long Activo. Monitoreando mercado...")
        self.send_telegram_msg("🚀 <b>Bot Long Pro Conectado</b>")
        
        while True:
            try:
                bm = BinanceSocketManager(self.client)
                async with bm.futures_multiplex_socket([f"{Config.SYMBOL.lower()}@kline_1m"]) as stream:
                    while True:
                        res = await stream.recv()
                        # Validación de seguridad WebSocket
                        if not res or 'data' not in res: continue
                        
                        k = res['data']['k']
                        print(f"[{datetime.now().strftime('%H:%M:%S')}] {Config.SYMBOL} ${k['c']} | OK", end='\r')
                        
                        if k['x']: # Al cierre de vela de 1m
                            klines = await self.client.futures_historical_klines(Config.SYMBOL, Config.INTERVAL, "1 day ago UTC")
                            ...
                            # Indicadores para Estrategia y Dashboard
                            ...
                            if stoch is not None: self.data['STOCH_K'] = stoch.iloc[:, 0]
                            ...
                            if not self.active_trade:
                                ...
                                if price > last['EMA_200'] and last['MACD'] > last['MACDs'] and 45 < last['RSI_14'] < 70:
                                    risk = last['ATR'] * Config.ATR_MULTIPLIER
                                    await self.execute_order('BUY', 'LONG', price, self.data.index[-1], price-risk, price+(risk*2))
                            else:
                                t = self.active_trade
                                if price >= t['tp'] or price <= t['sl']:
                                    await self.execute_order('SELL', 'CIERRE', price, self.data.index[-1])
            except Exception as e:
                logger.error(f"Error de red: {e}. Reintentando...")
                await asyncio.sleep(10)

async def main():
    bot = EliteTradingEngine()
    try:
        await bot.run()
    except (KeyboardInterrupt, asyncio.CancelledError):
        logger.info("⏹ Detención manual detectada.")
    finally:
        logger.info("Generando reporte final de sesión...")
        stats = bot.get_summary()
        if not bot.data.empty:
            path = TradingVisualizer.generate_dashboard(bot.data, bot.trades, stats)
            if path:
                bot.send_telegram_photo(path, stats)
        if bot.client:
            await bot.client.close_connection()
            logger.info("🔌 Conexión con Binance cerrada.")
