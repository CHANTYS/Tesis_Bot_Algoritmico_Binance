...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("trading_futures_long.log"), logging.StreamHandler()]
)
...
class Config:
    ...
    HISTORY_FILE = "historial_long_v1.json"
...
class TradingVisualizer:
    ...
    def generate_dashboard(df: pd.DataFrame, trades: List[Dict], stats: Dict):
        ...
        # 5. RESUMEN EJECUTIVO (KPIs) - IGUAL QUE EL BOT DE SHORTS
        ax5 = fig.add_subplot(gs[4])
        ax5.axis('off')
        summary_text = (
            f"RESUMEN DE SESIÓN: ELITE TRADING BOT (LONG)\n"
            f"{'='*60}\n"
        ...
        filename = f"reporte_long_live_pro.png"
...
class EliteTradingEngine:
    ...
    def send_telegram_photo(self, path, stats):
        ...
        caption = (
            f"🚀 <b>EXPERT REPORT: LONG SESSION</b>\n"
            f"----------------------------------\n"
    ...
    async def execute_order(self, side, reason, price, time, sl=None, tp=None):
        try:
            ...
            if reason == 'LONG':
                ...
                self.save_history()
                self.active_trade = None
                self.send_telegram_msg(f"✅ <b>CIERRE LONG</b>\nPnL Neto: ${t['pnl_neto']:.4f}")
    ...
    async def run(self):
        ...
        logger.info("🚀 Bot Long Iniciado...")
        self.send_telegram_msg("🚀 <b>Bot Long Conectado Correctamente</b>")
            ...
            try:
                ...
                async with bm.futures_multiplex_socket([f"{Config.SYMBOL.lower()}@kline_1m"]) as stream:
                    while True:
                        ...
                        # Validación de seguridad para evitar error 'data'
                ...
                logger.error(f"Error de red: {e}. Reintentando en 10s...")
